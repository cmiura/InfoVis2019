function Isosurfaces( volume, isovalue )
{
        ...
        for ( var y = 0; y < volume.resolution.y - 1; y++ )
        {
            for ( var x = 0; x < volume.resolution.x - 1; x++ )
            {
                ...
                for ( var j = 0; lut.edgeID[index][j] != -1; j += 3 )
                {
                    ...
                    var cell=volume.values[cell_index];
                    
                    face.color.set(new THREE.Color().setHex(cmap[cell][1]));
    ...
    {
        var v = new THREE.Vector3().addVectors( v0, v1 ).divideScalar( s );
        return new THREE.Vector3().addVectors(v,v1);
    }
